...
/**
 * Based on "Multifactorial Evolution: Towards Evolutionary Multitasking", by
...
public class MultiTaskingGA {
	...
	public MultiTaskingGA(List<Task> tasks, int numOfInd, double pOfMutation, int timeResetPopulation) {
		...
		this.population = new Population(numOfInd, tasks);
	}
	...
	public void run(int nN) {
		...
		for (int i = 0; i < ITERATIONS; i++) {
			...
			for (int j = 0; j < nN; j++) {
				...
				while (a == b)
					b = individuals.get(r.nextInt(individuals.size()));
	...
	private void selection() {
		population.setIndividuals(population.getIndividuals().stream().sorted((i1, i2) -> {
			Double di1 = i1.getScalarFitness();
			Double di2 = i2.getScalarFitness();
			return di1.compareTo(di2);
		}).limit(population.getNIndividual()).collect(Collectors.toList()));
	}
